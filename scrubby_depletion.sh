...
# Bowtie2 index basename (NOT the fasta)
SCRUBBY_INDEX="${REFERENCE_DIR}/controls"
...
for r1 in *RNA__PS_*R1_001.fastq.gz; do
    ...
    if [[ ! -f "$r2" ]]; then
        echo "  Skipping $sample: mate pair not found"
        continue
    fi
    ...
    scrubby reads \
        ...
        --threads "$THREADS" \
        -o \
        "${SCRUBBY_DIR}/${sample}__clean__R1.fq.gz" \
        "${SCRUBBY_DIR}/${sample}__clean__R2.fq.gz" \
